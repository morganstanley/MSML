...
RESULT_DIR="${4:-downstream_eval/longcodeqa/longcodeqa_4x}"
NUM_EXAMPLES="${5:-200}"
BATCH_SIZE="${6:-16}"
...
METHODS=("full" "no_context" "rag" "llmlingua2" "longcodezip" "longcodezip_with_pruner" "selective_context")
...
for method in "${METHODS[@]}"; do
    ...
    if [[ "${method}" == "rag" ]]; then
        ...
        ARGS+=(
            ...
            --selective_context_reduce_ratio 0.75
        )
    fi
