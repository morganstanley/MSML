#!/bin/bash
# Usage:
#   ./train/train_llm_v2_crf.sh <NUM_GPUS> <INPUT_FILE> [extra train args...]
#
# Example:
#   ./train/train_llm_v2_crf.sh 8 \
#     swe-pruner-training-dataset-py-v2-all-rubric-fixed.jsonl \
#     --log-dir llm_experiments/swe-pruner-py-v2-crf-final
#
# Dedicated wrapper for the v2 CRF path:
#   - max_length=4096
...
NUM_GPUS="${1:?Usage: train_llm_v2_crf.sh NUM_GPUS INPUT_FILE [extra args...]}"
INPUT_FILE="${2:?Usage: train_llm_v2_crf.sh NUM_GPUS INPUT_FILE [extra args...]}"
shift 2 || true
...
"$TORCHRUN_BIN" \
  ...
  --epochs 3 \
  --lr 1e-4 \
  --log-dir "$REPO_ROOT/llm_experiments/swe-pruner-py-v2-crf-final" \
