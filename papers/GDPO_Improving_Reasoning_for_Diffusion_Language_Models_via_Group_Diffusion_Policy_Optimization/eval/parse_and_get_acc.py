...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "gdpo")))
...
def parse_code_answers(json_path=None, json_data=None):
    ...
    for idx, item in enumerate(data.get("generations", [])):
        ...
        print(f"{id_value}/{idx}/{len(data['generations'])} current_reward {current_reward:.3f} correct {is_correct}")
...
def aggregate_results(directory="."):
    ...
    for json_file in json_files:
        ...
        if setup_name:
            ...
            elif any(name in setup_name for name in ["mbpp", "humaneval"]):
                (
                    correct,
                    processed,
                    detailed_results,
