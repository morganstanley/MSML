...
from likelihood_estimators import get_estimator
...
class GDPOTrainer(GRPOTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # TODO: Not using attention mask
        logps, _ = self.logp_estimator.get_log_likelihood(
            self.model, input_ids, logits_to_keep=logits_to_keep, seed=this_itr_mask_seed
        )
        ...
        coef_1 = torch.exp((logps - old_logps)/completion_ids.shape[-1]) # Divide by sequence length
    ...
    def _generate_completions_batch(self, prompt_ids, prompt_mask):
        """Generate completions in batches using the diffusion model."""
    ...
    def _process_completions(self, prompt_completion_ids, original_prompt_ids, device):
        """Process completions to extract completion IDs, masks, and generate mask seeds."""
        ...
        return prompt_ids, completion_ids, completion_mask, logits_to_keep, mask_seeds

    def _compute_seq_logps(self, prompt_completion_ids, logits_to_keep, mask_seeds):
        ...
        with torch.no_grad():
            if self.num_iterations > 1:
                ...
                old_logps, _ = self.logp_estimator.get_log_likelihood(
                    self.model, prompt_completion_ids_expanded, logits_to_keep=logits_to_keep, seed=this_itr_mask_seed
                )
        ...
        return old_logps, ref_logps

    def _compute_rewards_and_advantages(self, prompts, completion_ids, inputs, device):
        """Compute rewards from reward functions and calculate advantages."""
        ...
        self._current_rewards_per_func = rewards_per_func
        self._current_rewards = rewards
        self._current_std_grouped_rewards = std_grouped_rewards
        self._current_zero_std_ratio = zero_std_ratio
        self._current_completions_text = completions_text

        return advantages

    def _log_generation_metrics(self, completion_mask, prompts_text, completions_text, rewards):
        """Log generation metrics and completions."""
        mode = "eval" if self.control.should_evaluate else "train"

        completion_length = self.accelerator.gather_for_metrics(completion_mask.sum(1)).float().mean().item()
        self._metrics[mode]["completion_length"].append(completion_length)
        self._metrics[mode]["zero_std_ratio"].append(self._current_zero_std_ratio)
        ...
        for i, reward_func in enumerate(self.reward_funcs):
            ...
            mean_rewards = torch.nanmean(self._current_rewards_per_func[:, i]).item()
            self._metrics[mode][f"rewards/{reward_func_name}"].append(mean_rewards)
        self._metrics[mode]["reward"].append(rewards.mean().item())
        self._metrics[mode]["reward_std"].append(self._current_std_grouped_rewards.mean().item())
    ...
    def _generate_and_score_completions(
        ...
        This method coordinates the entire pipeline:
        1. Prepare prompts for generation
        2. Generate completions in batches
        3. Process completions and create masks
        4. Compute per-token log probabilities
        5. Compute rewards and advantages
        6. Log metrics and completions
        """
        device = self.accelerator.device

        # 1. Prepare prompts
        prompts, prompts_text, prompt_ids, prompt_mask = self._prepare_prompts_for_generation(inputs)

        # 2. Generate completions
        prompt_completion_ids = self._generate_completions_batch(prompt_ids, prompt_mask)

        # 3. Process completions and masks
        prompt_ids, completion_ids, completion_mask, logits_to_keep, mask_seeds = self._process_completions(
            prompt_completion_ids, prompt_ids, device
        )

        # 4. Compute log probabilities
        old_per_token_logps, ref_per_token_logps = self._compute_seq_logps(
            prompt_completion_ids, logits_to_keep, mask_seeds
        )

        # 5. Compute rewards and advantages
        advantages = self._compute_rewards_and_advantages(prompts, completion_ids, inputs, device)

        # 6. Log metrics
        self._log_generation_metrics(completion_mask, prompts_text, self._current_completions_text, self._current_rewards)
