...
class E2BProvider(CodeExecutionProvider):
    ...
    # changed num_parallel from 2 to 1
    def __init__(self, num_parallel: int = 1, e2b_router_url: Optional[str] = None):
        ...
        if self.e2b_router_url is not None:
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    ...
                    print(f"[WARN] Attempt {attempt+1} failed, sleeping {backoff:.2f}s: {e}")
                    time.sleep(backoff)
            
            if not executions:
                print("[ERROR] All attempts to run code via RoutedSandbox failed.")
                return [None] * len(scripts)
        ...
        async with semaphore:
            ...
            try:
                ...
                if execution is None:
                    print("[ERROR] All retries failed, returning 0.0 reward")
                    return 0.0

                for val in [
                    getattr(execution, "output", None),
                    getattr(execution, "text", None),
                    execution.logs.stdout[0] if execution.logs.stdout else "",
                ]:
                    try:
                        parsed = float(val.strip())
                        #print("[DEBUG] Parsed reward:", parsed)
                        return parsed
                    except Exception as e:
                        pass
                        #print(f"[DEBUG] Failed to parse {val!r}: {e}")

                return 0.0

            except Exception as e:
                print(f"[ERROR] Exception in `_run_script`: {e}")
                return 0.0
            finally:
                if sandbox is not None:
                    try:
                        ...
                        print(f"[ERROR] Failed to kill sandbox {sandbox_id}: {e}")
