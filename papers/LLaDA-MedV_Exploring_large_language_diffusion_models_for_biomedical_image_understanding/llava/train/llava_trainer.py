...
#####
### define helpful function to modify the configuration ###
...
class LLaVATrainer(Trainer):
    ...
    @staticmethod
    def load_projector_weights(model, projector_weight_path):
        
        if projector_weight_path is None:
            rank0_print("[INFO] No projector weights provided. Skipping load.")
            return model

        try:
            rank0_print(f"[INFO] Loading projector weights from {projector_weight_path}")
            weights = torch.load(projector_weight_path, map_location="cpu")
            ...
            model.get_model().mm_projector.load_state_dict(get_w(weights, "mm_projector"))
            rank0_print(f"[INFO] Successfully loaded projector weights from {projector_weight_path}.")
        except Exception as e:
            rank0_print(f"[ERROR] Failed to load projector weights: {e}")
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False) or (
            ...
            if self.args.local_rank == 0 or self.args.local_rank == -1:
                ### first need to modify the configuration
                patch_vlm_config(self.model)
