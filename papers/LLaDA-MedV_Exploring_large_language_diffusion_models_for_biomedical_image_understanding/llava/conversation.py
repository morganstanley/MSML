...
class Conversation:
    ...
    system: str ###Initial system prompt or context 
    roles: List[str] ##List of two roles 
    messages: List[List[str]] # List of [role, message] pairs. discussion history
    offset: int # Start index for display or processing.
    sep_style: SeparatorStyle = SeparatorStyle.SINGLE
    sep: str = "###" #Separators used in formatting.
    sep2: str = None #Separators used in formatting.
    version: str = "Unknown"

    tokenizer_id: str = ""
    tokenizer: Any = None
    # Stop criteria (the default one is EOS token)
    stop_str: Union[str, List[str]] = None #Criteria for stopping generation.
    # Stops generation if meeting any token in this list
    stop_token_ids: List[int] = None #Criteria for stopping generation.

    skip_next: bool = False

    def get_prompt(self, llava_llada_med_path = ''): ### building the full prompt
        ...
        if self.sep_style == SeparatorStyle.SINGLE:
        ...
        elif self.sep_style == SeparatorStyle.LLAMA_3: #### this is the default ####
            if self.tokenizer is None:
                ...
                elif self.version == "llava_llada_med":
                    self.tokenizer = AutoTokenizer.from_pretrained(llava_llada_med_path, trust_remote_code=True)
                else:
            ...
            for role, message in messages:
                if message:
                    if type(message) is tuple:
                        message, images = message ### first is message then is images
                        message = "<image>" * len(images) + message
    ...
    def append_message(self, role, message): ## append new [role, message] pair into message list
        self.messages.append([role, message])

    def process_image(self, image, image_process_mode, return_pil=False, image_format="PNG"):
        if image_process_mode == "Pad": ### how to process the image to target format based on requirements
...
conv_llada_plain = Conversation( ###the default this is for pretraining###
    system="",
    roles=("", ""),
    messages=[],
    version="llada_plain",
...
conv_llava_llada_med = Conversation(
    system = "You are a helpful medical language and vision assistant. " "You are able to understand the visual content that the user provides, " "and assist the user with a variety of tasks using natural language.",
    roles=("user", "assistant"),
    version="llava_llada_med",
    messages=[],
    offset=0,
    sep="<|eot_id|>",
    sep_style=SeparatorStyle.LLAMA_3,
...
conv_templates = {
    ...
    "llada_plain": conv_llada_plain, ### the default for pretraining
    "llava_llada": conv_llava_llada, ### the default for sft llada-v
    "llava_llada_med": conv_llava_llada_med, ### the default for sft llada_med
