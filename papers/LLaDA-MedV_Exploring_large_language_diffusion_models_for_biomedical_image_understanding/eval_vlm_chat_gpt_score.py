...
class LLMEvalPromptGenerator:
  ...
  role = 'Assistant'
#### the final line is used for rebuttal 
  @staticmethod
...
class ChatEvaluation:
  ...
  @staticmethod
  def eval(samples, score_txt, exp_name):
...
def main(args):
  ...
  with open(args.input_path) as f:
    for line in f:
      answer_data.append(json.loads(line))### list of dict

  question_data = [] ## load the ground_truth
  with open(args.question_input_path) as f:
    for line in f:
      question_data.append(json.loads(line)) ### list of dict
  ...
  for question, answer in zip(question_data, answer_data):
    ...
    question['ans2'] = answer['answer'] ### the generated answer key
  ...
  ChatEvaluation().eval(results, args.score_txt, args.exp_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--score_txt', default = '/scratch/xdong64/LLada_Vision/LLaDA-V/train/GPT_evaluation/generated_answer/debug.txt')
    parser.add_argument('--exp_name', default = 'DEBUG_EXP')
