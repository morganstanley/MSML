...
"""
script for training synthetic datasets using NNs
python train_sim.py --ds_str='ds1' --cuda=0 --n_replica=10 --train_size=500 --use_true_mean=0
"""
...
parser.add_argument("--n_replica", help='number of replica',type=int,default=5)
...
parser.add_argument("--output_folder", help='parent folder for output',type=str,default='output_sim')
...
def main():
    ...
    for replica_id in range(args.n_replica):
        ...
        if not args.use_true_mean:
            ...
            params = prepare_params(default_configs, configs, key = 'mean_net')
            ...
            # 1.2.2 test set
            x, mean_est = mean_estimator.run_on_testset(data_test)
        else:
        ...
        testset_predictions['x'], testset_predictions['mean_est'] = x, mean_est
        
        ## >>>>>>>>>>>>>>>>
        ## 1.3 train variance networks with different losses
        ## >>>>>>>>>>>>>>>>
        # 1.3.1 mse-loss based variance net
        print(f'****** training variance network with MSE loss ******')
        params = prepare_params(default_configs, configs, key = 'mse_variance_net')
        var_estimator_mse = train_network(params, var_data_train, var_data_val)
        testset_predictions['var_est_mse'] = var_estimator_mse.run_on_testset(data_test)[1]
        
        # 1.3.2 nll-loss based variance net, here for optimization purpose we are modeling the inverse of variance
        print(f'****** training variance network with NLL loss ******')
        params = prepare_params(default_configs, configs, key = 'nll_variance_net')
        var_estimator_nll = train_network(params, var_data_train, var_data_val)
        testset_predictions['var_est_nll'] = var_estimator_nll.run_on_testset(data_test)[1]
...
def prepare_params(default_configs, configs, key = 'mean_net'):
