...
def extend_cfg(cfg):
    ...
    # Config for PromptOT
    cfg.TRAINER.PROMPTOT = CN()
    cfg.TRAINER.PROMPTOT.N_CTX_VISION = 4  # number of context vectors at the vision branch
    cfg.TRAINER.PROMPTOT.N_CTX_TEXT = 4  # number of context vectors at the language branch
    cfg.TRAINER.PROMPTOT.CTX_INIT = "a photo of a"  # initialization words
    cfg.TRAINER.PROMPTOT.PREC = "fp16"  # fp16, fp32, amp
    cfg.TRAINER.PROMPTOT.PROMPT_DEPTH_VISION = 9  # Max 12, minimum 0, for 0 it will be using shallow IVLP prompting (J=1)
    cfg.TRAINER.PROMPTOT.PROMPT_DEPTH_TEXT = 9  # Max 12, minimum 0, for 0 it will be using shallow IVLP prompting (J=1)
    cfg.TRAINER.PROMPTOT.TEXT_LOSS_WEIGHT = 25
    cfg.TRAINER.PROMPTOT.IMAGE_LOSS_WEIGHT = 10
    cfg.TRAINER.PROMPTOT.GPA_MEAN = 15
    cfg.TRAINER.PROMPTOT.GPA_STD = 1
    cfg.DATASET.SUBSAMPLE_CLASSES = "all"  # all, base or new
...
def main(args):
    ...
    if args.eval_only:
        ...
        # trainer.load_model(args.model_dir, epoch=args.load_epoch) #args.load_epoch
        # trainer.test()
        # args.load_epoch= 1
        for epoch in [args.load_epoch+1]:#range(1,args.load_epoch+1): # you can evaluate every epoch
            trainer.load_model(args.model_dir, epoch=epoch) #args.load_epoch
            trainer.test()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--div", type=float, default=0., help="impose lamda"
    )
