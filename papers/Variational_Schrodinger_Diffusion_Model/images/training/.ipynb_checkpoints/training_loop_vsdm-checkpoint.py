...
def training_loop(
    ...
    fnetwork_kwargs      = {},       # Options for model and preconditioning.
    ...
    fnet = dnnlib.util.construct_class_by_name(**fnetwork_kwargs, **interface_kwargs) # subclass of torch.nn.Module 
    fnet.train().requires_grad_(True).to(device) 
            ...
            for round_idx in range(num_accumulation_rounds):
                with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                    if method == 'fbsde':
                        ...
                        images = images.to(device).to(torch.float32) / 127.5 - 1
                        labels = labels.to(device)
                        loss = loss_fn.vsdm_back_loss(net=ddp, fnet=fema, images=images, labels=labels, augment_pipe=augment_pipe)
