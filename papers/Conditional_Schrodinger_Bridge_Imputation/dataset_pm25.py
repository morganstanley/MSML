...
def get_dataloader(
    ...
    dataset = PM25_Dataset(mode="train", eval_length=eval_length, target_dim=target_dim,
        validindex=validindex)
    dataset_test = PM25_Dataset(mode="test", eval_length=eval_length, target_dim=target_dim,
        validindex=validindex)
    dataset_valid = PM25_Dataset(mode="valid", eval_length=eval_length, target_dim=target_dim,
        validindex=validindex)

    print('train/test/val num samples', len(dataset), len(dataset_valid), len(dataset_test))
    if return_dataset:
        return dataset, dataset_valid, dataset_test
    else:
        scaler = torch.from_numpy(dataset.train_std).float().to(device)
        mean_scaler = torch.from_numpy(dataset.train_mean).float().to(device)
