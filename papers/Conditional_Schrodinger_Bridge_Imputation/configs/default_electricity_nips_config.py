...
def get_electricity_nips_default_configs():
  ...
  # config.input_size = (370, 48+24)  # (K,L)
  # config.target_dim_range = (48*3, 48*4)
  config.input_size = (8, 48+24)  # (K,L)
...
def get_Unetv2_config(input_size=None):
  ...
  config.num_channels = 32  # 128 <-- Due to GPU memory limitation.
  config.num_norm_groups = 32  # num_channels is divisible by num_norm_groups.
  config.dropout = 0.0
  config.channel_mult = (1,1) # (1, 1, 1)  # (1, 1, 2, 2)  # 128 <-- Due to GPU memory limitation.
...
def get_Transformerv2_config(input_size=None, output_layer='conv1d'):  # Default model.
  ...
  config.layers = 4  # 4 <-- Due to GPU memory limitation.
  config.nheads = 4
  config.channels = 64
  config.diffusion_embedding_dim = 128
  config.timeemb = 32  # 128 <-- Due to GPU memory limitation.
