...
from models.Transformer.diff_models import diff_CSDI


def build_transformerv3(config, interval, zero_out_last_layer):
...
class Transformerv3(nn.Module):
    """Similar to Transformerv2, instead of using inputx = cat([cond_obs, noisy_target])
    Transformerv3 does inputx = cond_obs + noisy_target as these are complementary.
    """
    def __init__(
        self,
        input_size,
