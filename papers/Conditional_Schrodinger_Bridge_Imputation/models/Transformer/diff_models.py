...
class diff_CSDI(nn.Module):
    def __init__(self, config, inputdim, zero_out_last_layer=True):
        ...
        # The order of adding parameters matter. The output parameters of the original output layer
        # is here.
        if self.output_layer in ['conv1d_relu', 'conv1d_silu', 'conv1d_leaky_relu0.01',
                'conv1d_leaky_relu0.1']:
        ...
        if self.output_layer in ['conv1d']:
        ...
        elif self.output_layer not in ['conv1d_relu', 'conv1d_silu', 'conv1d_leaky_relu0.01',
                'conv1d_leaky_relu0.1']:
            raise NotImplementedError(f'Unknown output_layer {self.output_layer}')

        if zero_out_last_layer and self.output_layer in ['conv1d_relu', 'conv1d_silu',
                'conv1d_leaky_relu0.01', 'conv1d_leaky_relu0.1']:
            self.output_projection2 = zero_module(self.output_projection2)
        elif zero_out_last_layer:
    ...
    def forward(self, x, diffusion_step=None, window_embedding=None):
        ...
        if self.output_layer in ['conv1d_relu', 'conv1d_silu', 'conv1d_leaky_relu0.01',
                'conv1d_leaky_relu0.1']:  # Improper activation function.
            # The output projection causes divergence calulation issue.
            x = x.reshape(B, self.channels, K * L)
            x = self.output_projection1(x)  # (B,channel,K*L)
            if self.output_layer == 'conv1d_relu':
                x = F.relu(x)
            elif self.output_layer == 'conv1d_silu':
                x = F.silu(x)
            elif self.output_layer == 'conv1d_leaky_relu0.01':
                x = F.leaky_relu(x, negative_slope=0.01)
            elif self.output_layer == 'conv1d_leaky_relu0.1':
                x = F.leaky_relu(x, negative_slope=0.1)
            x = self.output_projection2(x)  # (B,1,K*L)
            x = x.reshape(B, 1, K, L)
        elif self.output_layer == 'conv1d':
            # The output projection causes divergence calulation issue.
            x = x.reshape(B, self.channels, K * L)
            x = self.output_projection1(x)  # (B,channel,K*L)
            x = F.leaky_relu(x, negative_slope=0.1)
