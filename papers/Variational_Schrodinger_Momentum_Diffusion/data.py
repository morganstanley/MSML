...
def build_boundary_distribution(opt, Cov=None):
    ...
    pvelocity = build_prior_sampler(opt, opt.samp_bs)
    pdata = build_data_sampler(opt, opt.samp_bs)
    return pdata, prior, pvelocity
...
def build_prior_sampler(opt, batch_size, Cov=None, means=None):
    if Cov == None:
...
class MixMultiVariateNormal:
    ...
    def sample(self):
        ...
        samples=torch.cat(samples,dim=0) * 0.25
