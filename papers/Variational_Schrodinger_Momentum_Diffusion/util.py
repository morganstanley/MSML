...
def is_toy_dataset(opt):
    return opt.problem_name in ['gaussian', 'gmm','checkerboard', 'spiral']
...
def evaluate_stage(opt, stage, metrics):
    ...
    if metrics is not None:
        return [k in metrics for k in ['FID', 'snapshot']]
    match = lambda freq: (freq>0 and stage%freq==0)
    return [match(opt.FID_freq), match(opt.snapshot_freq)]
...
def save_toy_npy_traj(opt, fn, traj, n_snapshot=None, direction=None):
    ...
    if n_snapshot is None: # only store t=0
        ...
        for ax, step in zip(axs, sample_steps):
            ...
            if opt.problem_name in ['spiral', 'checkerboard', 'gmm']:
                #pass
                if step == sample_steps[-1]:
                    ax.set_xlim([-4, 4])
                    ax.set_ylim([-4, 4])
                    mu_x, mu_y = np.mean(traj[:,step,0]), np.mean(traj[:,step,1])
                    var_x = np.mean(traj[:,step,0]**2) - np.mean(traj[:,step,0])**2
                    var_y = np.mean(traj[:,step,1]**2) - np.mean(traj[:,step,1])**2
                    print(yellow(f'{direction} mean {mu_x:.2f} {mu_y:.2f} var {var_x: .2f} {var_y: .2f}'))
            else:
                ax.set_xlim(*lims)
                ax.set_ylim(*lims)
            ax.xaxis.set_tick_params(labelsize=35)
            ax.yaxis.set_tick_params(labelsize=35)
            ax.set_title('t={:.2f}'.format(step/(total_steps-1)*opt.T), fontsize=35)
        fig.tight_layout()

    if 'ode' in fn_npy.split('/')[-1]:
        np.save(fn_npy, traj)
    else:
        plt.savefig(fn_pdf)
        plt.clf()
...
def snapshot(opt, img, stage, direction):
    ...
    n = 36 if opt.compute_FID else 24**2
...
def check_duplication(opt):
    ...
    runs_dir='others/runs/'+opt.log_fn
