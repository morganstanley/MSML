...
def compute_div_gz(opt, dyn, ts, xs, policy, return_zs=False):
    ...
    if opt.diffusion == 'CLD':
        gzs = torch.cat([torch.zeros_like(gzs,), gzs], dim=-1)
...
def compute_sb_nll_alternate_train(opt, dyn, ts, xs, zs_impt, policy_opt, return_z=False):
    ...
    with torch.enable_grad():
        ...
        if opt.diffusion == 'CLD':
            zs = torch.cat([torch.zeros_like(zs,), zs], dim=-1)
            zs_impt = torch.cat([torch.zeros_like(zs_impt,), zs_impt], dim=-1)
