...
from sde_ld import compute_vp_diffusion


class LinearPolicy(torch.nn.Module):
    def __init__(self, data_dim=2, hidden_dim=64, time_embed_dim=128, net_name='Linear', t_truncate=1.):
        super(LinearPolicy,self).__init__()

        self.data_dim = data_dim
        self.net_name = net_name
        self.time_embed_dim = time_embed_dim
        self.t_truncate = t_truncate
    ...
    def At(self, t):
        if 'static' not in self.net_name.lower():
            ...
            self.t_out = torch.clip(self.t_out, min=1./self.t_truncate, max=self.t_truncate)
        else:
            self.t_out = torch.ones(t.shape[0])
        return torch.einsum('ij,t->tij', self.A, self.t_out)

    def forward(self, x, t, beta_min=0.1, beta_max=10., beta_r=1., interval=100., baseline=False):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        :baseline only aims to simplify line 77 in policy.py
        """
        # make sure t.shape = [T]
        if len(t.shape) == 0:
            t = t[None]
