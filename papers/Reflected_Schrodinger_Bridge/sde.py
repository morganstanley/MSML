...
class BaseSDE(metaclass=abc.ABCMeta):
    ...
    def propagate_default(self, t, x, z, direction, f=None, dw=None, dt=None):
        ...
        # x_new = x + (f + g*z)*dt + g*dw
        # x_new = cube.reflect(x_new)
        # return x_new
    ...
    def compute_nll(self, samp_bs, ts, z_f, z_b):
        """Yu: SCORE-BASED GENERATIVE MODELING THROUGH STOCHASTIC DIFFERENTIAL EQUATIONS, Appendix D.2
        ...
        for idx, t in enumerate(tqdm(ts,desc=util.yellow("Propagating Dynamics..."))):
            ...
            x = self.propagate(t, x, z, direction, f=f)  # Yu ERROR: should be ODE not SDE.
            # x = dx_dt * self.dt
...
class VPSDEv3(BaseSDE):
    """The diffusion term is fixed as an increasing function.
    While we tune the forward function.
    """
    def __init__(self, opt, p, q):
        super(VPSDEv3, self).__init__(opt, p, q)
        self.b_min=opt.beta_min
        self.b_max=opt.beta_max
    ...
    def _f(self, x, t):
        scale = -0.5*(self.b_min+t*(self.b_max-self.b_min))
        return scale*x

    def _g(self, t):
        return compute_vp_diffusion(t, self.sigma_min, self.sigma_max)
...
class RVESDE(BaseSDE):
    def __init__(self, opt, p, q):
        super(RVESDE, self).__init__(opt, p, q)
        self.s_min=opt.sigma_min
        self.s_max=opt.sigma_max
    ...
    # def _g(self, t):
    #     """
    #     compute_ve_diffusion(t, self.s_min, self.s_max)
    #     = compute_sigmas(t, s_min, s_max) * compute_ve_g_scale(s_min, s_max)
    #     = s_min * (s_max/s_min)**t  *  np.sqrt(2*np.log(s_max/s_min))
    #     """
    #     s_min, s_max = self.s_min, self.s_max
    #     sigma = s_min * (s_max/s_min)**t
    #     diffusion = sigma * np.sqrt(2*np.log(s_max/s_min))
    #     return diffusion
    ...
    def sample_traj_debug(
            ...
            num_samples=None,
            save_traj=True):
        """Debug for backward sampling RVE + nscnpp."""
        # first we need to know whether we're doing forward or backward sampling
        opt = self.opt
        direction = policy.direction
        ...
        x = init_dist.sample(num_samples=num_samples).to(opt.device) # [bs, x_dim]
        torch.manual_seed(3)
        x = torch.rand_like(x).to(opt.device)

        _ts = tqdm(ts, ncols=120, file=sys.stdout, desc=util.yellow(f"{direction} SDE sampling..."))
        for idx, t in enumerate(_ts):
            vec_t = torch.ones(x.shape[0], device=t.device) * t
            time_cond = opt.sigma_min * (opt.sigma_max / opt.sigma_min) ** vec_t
            score = policy.net(x, time_cond)
            g = self.g(t)
            # dw = self.dw(x)
            dw = np.sqrt(self.dt) * torch.randn_like(x)
            x = x + g**2 * score * self.dt + g*dw
            x = cube.reflect(x)
...
class RVPSDE(BaseSDE):
    def __init__(self, opt, p, q):
        super(RVPSDE,self).__init__(opt, p, q)
        self.b_min=opt.beta_min
        self.b_max=opt.beta_max

    def _f(self, x, t):
        """WARNING: this is mosified from VPSDE where the mean center is set as [0.5,0.5,...]
        because of the cube domain [0,1]^d. The SDE converges to 0.5.
        """
        return compute_vp_drift_coef(t, self.b_min, self.b_max)*(x-0.5)
...
def compute_ve_xs_label_tdim1(opt, x0, sigmas, samp_t_idx, return_scale):
    ...
    # score_of_p = -1/sigma_t^2 (x_t - x_0) = -noise/sigma_t^2
    # dx_t = g dw_t, where g = sigma_t * g_scaling
    # hence, g * score_of_p = - noise * g_scaling
    g = compute_ve_g_scale(s_min, s_max)
    label = - noise * g
    if return_scale:
        return analytic_xs, label, g  # score input, target score, scale
    else:
...
def compute_rve_xs_label(opt, x0, sigmas, samp_t_idx, return_scale=False):
    if samp_t_idx.dim() == 1:
        pass  # TODO: double check basic version.
        # return compute_rve_xs_label_tdim1(opt, x0, sigmas, samp_t_idx, return_scale)
    elif samp_t_idx.dim() == 2:
        return compute_rve_xs_label_tdim2(opt, x0, sigmas, samp_t_idx, return_scale)


def compute_rve_xs_label_tdim1(opt, x0, sigmas, samp_t_idx, return_scale):
    """ return xs.shape == [batch_x, *x_dim]
    samp_t_idx (batch_t) is the same for different batch_x samples.
    """
    s_max = opt.sigma_max
    ...
    sigma_t = sigmas[samp_t_idx].reshape(1,batch_t,*([1,]*len(x_dim))) # shape = [1,batch_t,1,1,1]
    analytic_xs = sigma_t * noise + x0[:,None,...]
    analytic_xs = cube.reflect(analytic_xs)
    mean = x0
    sigma_t = sigma_t.reshape(batch_x*batch_t)
    label = cube.score_hk(analytic_xs, mean, sigma_t)

    # score_of_p = -1/sigma_t^2 (x_t - x_0) = -noise/sigma_t; x_t = x_0 + sigma_t*noise
    # dx_t = g dw_t, where g = sigma_t * g_scaling
    # hence, score_of_p * g = - noise /sigma_t * g
    g = compute_ve_g_scale(s_min, s_max)
    label = - label * g
    if return_scale:
        return analytic_xs, label, g
...
def compute_rve_xs_label_tdim2(opt, x0, sigmas, samp_t_idx, return_scale):
    ...
    sigma_t = sigmas[samp_t_idx].reshape(batch_x,batch_t,*([1,]*len(x_dim)))
    analytic_xs = sigma_t * noise + x0[:,None,...]
    analytic_xs = analytic_xs.reshape(batch_x*batch_t,*x_dim)
    analytic_xs = cube.reflect(analytic_xs)
    mean = x0.unsqueeze(1).repeat(1,batch_t,*([1,]*len(x_dim)))
    mean = mean.reshape(batch_x*batch_t,*x_dim)
    sigma_t = sigma_t.reshape(batch_x*batch_t)
    score_hk = cube.score_hk(analytic_xs, mean, sigma_t)
    score_hk = score_hk.reshape(batch_x, batch_t, *x_dim)
    analytic_xs = analytic_xs.reshape(batch_x, batch_t, *x_dim)
    ...
    # hence, g * score_of_p = - noise * g_scaling
    g = compute_ve_g_scale(s_min, s_max)
    sigma_t = sigma_t.reshape(batch_x,batch_t,*([1,]*len(x_dim)))
    # score_hk = score_of_p, but needs to be rescaled.
    label = score_hk * sigma_t * g
...
def get_xs_label_computer(opt, ts):
    ...
    elif opt.sde_type == 'rve':
        sigmas = compute_sigmas(ts, opt.sigma_min, opt.sigma_max)
        fn = compute_rve_xs_label
        kwargs = dict(opt=opt, sigmas=sigmas)
