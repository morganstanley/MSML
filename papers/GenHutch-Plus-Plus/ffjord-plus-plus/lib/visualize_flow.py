...
def scale_global_axis(x_scale, y_scale):
    global SCALE
...
def plt_flow_density(prior_logdensity, inverse_transform, ax, npts=100, memory=100, title="$q(x)$", device="cpu"):
    side_x = np.linspace(LOW * SCALE[0], HIGH * SCALE[0], npts)
    side_y = np.linspace(LOW * SCALE[1], HIGH * SCALE[1], npts)
...
def plt_flow_samples(prior_sample, transform, ax, npts=100, memory=100, title="$x ~ q(x)$", device="cpu", return_sample=False):
    ...
    ax.hist2d(zk[:, 0], zk[:, 1], range=[[LOW * SCALE[0], HIGH * SCALE[0]], [LOW * SCALE[1], HIGH * SCALE[1]]], bins=npts)
    ax.invert_yaxis()
...
def visualize_transform(
    ...
    memory=100, device="cpu", return_sample=False
):
    ...
    if transform is not None:
        flow_sample = plt_flow_samples(prior_sample, transform, ax, npts=npts, memory=memory, device=device, return_sample=return_sample)
