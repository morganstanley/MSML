...
1. **torch.compile** — can give large throughput gains, but is **NOT default-safe** in this repo.
   - We have seen systemic failures with TorchInductor CUDA graphs ("tensor output overwritten") when models do lazy buffer/cache init inside a compiled forward (often RoPE/rotary caches).
   - Treat compile as **opt-in**: only propose `compile=true` if the worker also disables cudagraphs and pre-initializes caches.
...
4. **Prune the queue** — Review `to_implement` experiments in light of new results:
   ...
   - If compile-enabled experiments are repeatedly crashing, cancel compile variants until the underlying issue is fixed
...
5. **Propose 2-5 new experiments** per turn:
   ...
   - Config JSON format: `{"architecture": "llama", "n_layer": 8, "n_head": 8, "n_embd": 512, "ffn_type": "swiglu", "norm_type": "rmsnorm", "pos_encoding": "rope", "optimizer": "adamw", "learning_rate": 5e-4, "lr_schedule": "cosine", "warmup_steps": 1000, "weight_decay": 0.1, "batch_size": 64, "seq_len": 1024, "compile": false, "dtype": "bf16"}`
...
6. **Update playbook.md** with compressed wisdom:
   ...
   - Known failure modes (OOM configs, NaN-producing settings, compile crashes, over-budget param counts)
...
- On your first turn, propose a diverse initial batch: one architecture comparison (depth vs width), one optimizer comparison (AdamW vs Muon), one with SwiGLU + RMSNorm + RoPE (LLaMA-style), one aggressive combined config.
- Treat `torch.compile` as opt-in until proven stable.
