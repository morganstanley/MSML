...
3. **BUILD IN `harness/`.** All framework code goes in `harness/`:
   ...
   - `data_prep.py` — Data preparation (PleIAs SYNTH):
     - Reads SYNTH **parquet shards** (`synth_001.parquet` … `synth_500.parquet`) from the corpus directory specified in config
     - Extracts training text by concatenating `synthetic_answer` (optionally `query`) with clear separators
     - Supports sampling: choose a subset of shards and/or cap rows per shard (the full 220GB dataset is too large for the time budget)
   ...
   - `config.py` — Default configuration:
     ...
     - Data settings (dataset path, parquet shard selection/sampling, num_workers, pin_memory)
