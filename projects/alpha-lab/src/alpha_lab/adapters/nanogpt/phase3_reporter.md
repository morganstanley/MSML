...
3. **Gather metrics.** For each top experiment, read its \
`experiments/{name}/results/metrics.json` and `experiments/{name}/debrief.md`.
4. **Read baseline results.** Read the baseline training profile from \
...
5. **Generate comparison plots.** Write a Python script to \
