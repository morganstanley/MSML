...
1. **READ CONTEXT FIRST.** Start by reading `{workspace}/learnings.md` and `{workspace}/data_report/` files to understand the dataset, its columns, target variable, value distribution, and quirks. Also read `{workspace}/agenda.md` if present — the framework you build should support the user's stated success criteria and respect any out-of-scope items.
   ...
   - `engine.py` -- Evaluation engine. Exposes a single `evaluate` function with this signature:
     ...
     def evaluate(
         ...
         *,
         ...
         Returns:
             A dict of val-side metrics (the same data written to
             ``results/metrics.json``). Test metrics live under
             ``private/`` and MUST NEVER appear in the return value of
             this function — including them, even under a ``test/`` prefix
             or any other guise, leaks held-out signal to every downstream
             agent that captures the return value.
         """
     ```
     ...
     - Fit-predict the Strategy on test using the same trained model, compute metrics.
     ...
     - Write the combined val+test metrics to `{workspace}/private/experiments/{experiment_name}/metrics.json` with metric names prefixed by `val/` and `test/` (e.g. `{"val/mse": ..., "val/mae": ..., "test/mse": ..., "test/mae": ...}`). Create the parent directory if missing.
     - The `private/` directory and its contents must never be referenced from any code outside `engine.py`.
   - `metrics.py` -- Regression metrics: MSE, MAE, R-squared. Takes predictions + ground truth, returns a dict.
   - `baselines.py` -- Baseline strategies: mean predictor, median predictor, linear regression. These set the performance floor.
