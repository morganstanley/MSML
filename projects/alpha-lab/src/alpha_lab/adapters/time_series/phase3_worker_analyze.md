...
3. **Read SLURM output**: `experiments/{name}/slurm_*.out`
4. **Read results**: `experiments/{name}/results/metrics.json` and any plots in `experiments/{name}/results/`
5. **Verify model artifacts**: Check that `experiments/{name}/results/best_model/` exists and contains the saved model (weights, scalers, config). If missing, note this as a deficiency — the experiment is incomplete without saved weights. Include the model path in the results JSON.
