...
- `{workspace}/experiments/{name}/results/metrics.json` MUST contain at least: accuracy, f1, precision, recall.
...
- **Cross-validation is allowed** within an experiment, but resource constraints (CPU time, memory) apply to the entire CV process, not individual folds.
